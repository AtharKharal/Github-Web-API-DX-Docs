...
```http
GET {{baseUrl}}/issues?filter=assigned&state=open&labels=<string>&sort=created&direction=desc&since=<string>&collab=<boolean>&orgs=<boolean>&owned=<boolean>&pulls=<boolean>&per_page=30&page=1
```
...
| `filter` | `string` | `Query` | `No` | Indicates which sorts of issues to return. Can be one of:  
...
| `collab` | `string` | `Query` | `No` |  |

| `orgs` | `string` | `Query` | `No` |  |

| `owned` | `string` | `Query` | `No` |  |

| `pulls` | `string` | `Query` | `No` |  |
